...
`forge-gfx-palette-mark` is centered on graphics work in Dart. The technical target is: Design a Dart verification harness for palette systems, covering incremental indexing, append-only fixtures, and failure-oriented tests.
...
The score is deterministic. The current threshold is 161. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 5.
